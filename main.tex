...
\begin{itemize}
    ...
    \begin{tikzpicture}[auto, node distance=2cm, >=triangle 45]
        \draw % set the nodes :
            ...
            node at (-0.3,-1.7) {$-$};
        % link the nodes [vecArrow] and [->] 
        \draw[vecArrow](KZ) -- (suma);
        \draw[vecArrow](KR) -- (suma);
        \draw[vecArrow](suma) -- (ZF);
...
\uline{Ermittlung von $\uline{K}$}\begin{enumerate}
    ...
    Orientierung an PT$_2$-Parametern, \fbox{s. BB. ORS 2-23} $\Rightarrow$ Forderungen an den Polbereich $\Gamma$ motiviert:\begin{enumerate}
        ...
        \fbox{$-\alpha_{\omega 0}\leq-a$} \quad (a>0)\\
        Approximation der rechten Grenze\\
        \uline{Hyperbel $C_1$}: \quad \fbox{$\displaystyle\frac{\eta^2}{a^2}-\frac{\omega^2}{b^2}=1$} \quad (b=a xxxxxxxx $\psi_m$)
...
\uline{Anmerkungen}:\begin{itemize}
    ...
    \fbox{$E\{\tilde{s}\}=E_y\bigg\{E_s\{s|y\}\bigg\}\stackrel{!}{=}E\{s\}$} \quad (s. auch sp"ater)
...
\begin{enumerate}[label=(\Alph*)]
    ...
    f"ur Dynamik: Momente 1. und 2. Ordnung zu betrachten (Gau"s-Zustandsvariablen)\begin{enumerate}
        ...
        $\Rightarrow$ \fbox{$\uline{P}_{\uline{xx}}(k+1)=\uline{\Phi}(k+1,k)\uline{P}_{\uline{xx}}(k)\uline{\Phi}^T(k+1,k)+\uline{L}(k)\uline{P}_{\uline{xx}}(k)\uline{L}^T(k)$} \quad (8)
    \end{enumerate}
